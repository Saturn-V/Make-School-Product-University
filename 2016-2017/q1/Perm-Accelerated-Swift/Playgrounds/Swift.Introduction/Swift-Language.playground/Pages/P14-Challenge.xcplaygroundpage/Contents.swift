...
enum Rank: Int, Comparable {
    ...
    static func < (lCard: Rank, rCard: Rank) -> Bool {
        return lCard.rawValue < rCard.rawValue
    }
}
...
enum Suit: Int, Comparable {
    ...
    static func < (lCard: Suit, rCard: Suit) -> Bool {
        return lCard.rawValue < rCard.rawValue
    }
}
...
class Deck {
    ...
    func drawOne() -> Card? {
        let rand = Int(arc4random_uniform(deckCount))
        let drawnCard = deck[rand]
 ...
 */
/* Place your code here! */
func compareCards(playerCard: Card, computerCard: Card) -> Card {
    if (playerCard.rank < computerCard.rank) {
        print("The computer won with the \(computerCard.rank) of \(computerCard.suit)")
        return computerCard
    } else if(computerCard.rank < playerCard.rank) {
        print("The player won with the \(playerCard.rank) of \(playerCard.suit)")
        return playerCard
...
for _ in 1...52 {
    let playerCard = playerDeck.drawOne()
    let computerCard = computerDeck.drawOne()
    compareCards(playerCard: playerCard!, computerCard: computerCard!)
}
